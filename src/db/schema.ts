...
export const productTypeItem = pgTable("product_type_item", {
  ...
  productItemId: integer("product_item_id").notNull().references(() => productItem.id)
})
...
export const paymentMethod = pgTable("paymen_method", {
  id: serial("id").primaryKey(),
  name: varchar("name").notNull()
})
...
export const productTypeRelations = relations(productType,
  ({ many }) => ({
...
export const productItemRelations = relations(productItem, 
  ({ many }) => ({
    productTypeItems: many(productTypeItem)
  })
)

export const productTypeItemRelations = relations(productTypeItem, 
  ({ one }) => ({
    productType: one(productType, {
...
export const saleProductRelations = relations(saleProduct,
  ({ one }) => ({
    sale: one(sales, {
